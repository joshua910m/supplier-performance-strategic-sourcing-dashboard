...
def render_kpi_cards(component_summary: pd.DataFrame, supplier_summary: pd.DataFrame) -> None:
    ...
    cols = st.columns(6)
    for col, (label, value, help_text) in zip(cols, kpi_items):
        with col:
            st.markdown(
                f"""
...
def build_supplier_risk_scatter(supplier_summary: pd.DataFrame) -> alt.Chart:
    ...
    return (
        ...
        .encode(
            x=alt.X("avg_lead_time:Q", title="Average Lead Time (days)"),
            y=alt.Y("defect_rate:Q", title="Defect Rate"),
            size=alt.Size("spend:Q", title="Spend", scale=alt.Scale(range=[180, 1800])),
            color=alt.Color("decision:N", title="Decision", scale=color_scale),
            tooltip=[
                alt.Tooltip("supplier:N", title="Supplier"),
                alt.Tooltip("spend:Q", title="Spend", format="$,.0f"),
                alt.Tooltip("defect_rate:Q", title="Defect Rate", format=".1f"),
                alt.Tooltip("avg_lead_time:Q", title="Average Lead Time", format=".1f"),
                alt.Tooltip("supplier_risk_score:Q", title="Supplier Risk Score", format=".1f"),
